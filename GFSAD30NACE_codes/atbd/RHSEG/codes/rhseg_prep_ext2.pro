pro RHseg_prep_ext2, regname
  compile_opt idl2
  e=envi(/headless)
  
  in_dir='/scratch/rm885/gdrive/sync/RHseg/RHSEG_tiles/'+regname
  out_dir='/scratch/rm885/gdrive/sync/RHseg/processing/RHSeg2/'+regname
  ...
  for i=0,n_elements(filelist)-1 do begin
    ...
    img_sub=envisubsetraster(img,bands=[1,2,3,4,5,7,8,9])
    ...
    spatialref = spref, data_type=2,nrows=nl, ncolumns=ns)
    maskraster.save
    e.ExportRaster, maskraster, mask_file, 'tiff'
    maskraster.close
    if file_test(mask_envi) gt 0 then file_delete,mask_envi
    if file_test(mask_envi+'.hdr') gt 0 then file_delete,mask_envi+'.hdr'
...
;    endif
    ...
    print,''
    free_lun,llun
    tempjob=strsplit(spawn_out, ' ',/extract)
    jobid=[jobid,long(tempjob[3])]
  endfor
...
for i=0, n_elements(filelist)-1 do if file_test(filelist[i]) gt 0 then file_delete, filelist[i]

print,'Done!'

print, 'Job list: '+ strjoin(strtrim(jobid,2),' ')
print,'----------------------------------------------------------------------'
close,/all
;return, jobid
end
